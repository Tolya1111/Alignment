...
class Reward_Model_OR(Trainer):
    ...
    def compute_loss(
        ...
        if type(model) == torch.nn.DataParallel:
            logits_chosen = model.module.model(
                input_ids=inputs["input_ids_chosen"],
                attention_mask=inputs["attention_mask_chosen"],
            )['last_hidden_state']
            logits_rejected = model.module.model(
                input_ids=inputs["input_ids_rejected"],
                attention_mask=inputs["attention_mask_rejected"],
            )['last_hidden_state']
            rewards_chosen = model.module.reward(logits_chosen[:, -1, :])
            rewards_rejected = model.module.reward(logits_rejected[:, -1, :])
        else:
            logits_chosen = model.model(
                input_ids=inputs["input_ids_chosen"],
                attention_mask=inputs["attention_mask_chosen"],
            )['last_hidden_state']
            logits_rejected = model.model(
                input_ids=inputs["input_ids_rejected"],
                attention_mask=inputs["attention_mask_rejected"],
            )['last_hidden_state']
            rewards_chosen = model.reward(logits_chosen[:, -1, :])
            rewards_rejected = model.reward(logits_rejected[:, -1, :])
    ...
    def prediction_step(
        ...
        logits = torch.stack(logits).squeeze(2)
